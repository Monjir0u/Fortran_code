module parameters
    ...
    double precision, parameter :: timeMeasure = 0.020d0 ! データ計測時間[ns]
    ...
    double precision, parameter :: angCon = 0.05d0 ! 相互作用強さ

    character(len=20) :: dir_name = 'yuragi2'  ! ファイル名
...
module variable
    ...
    double precision :: zeroHeat(TYPMOL)
...
program main
    ...
    subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
        ...
        integer :: cnt(TYPMOL) ! ensembleHeatの配列に入れる用のカウント
        integer :: stp
       ...
       ! 異なる分子同士の影響  ! Ar-Ptの処理    配列は1が上Pt, 2がAr, 3がしたPt
        ...
        if(stpNow > stpScaling + stpRelax) then
            do j = 1, TYPMOL
                ...
                if(mod(stp, plot) == 1) then
                    zeroHeat(j) = heatInterface(j)
                    cnt(j) = 0
                end if

                scalarHeat(j) = scalarHeat(j) + heatInterface(j) * zeroHeat(j)

                cnt(j) = cnt(j) + 1
                if(stp < 2000) then
                    write(69,'(3I7, 2E15.7)') stpNow, stp, count, zeroHeat, heatInterface(j)
                end if
                ensembleHeat(cnt(j),j) = ensembleHeat(cnt(j),j) + scalarHeat(j)
            end do
        end if
    ...
    subroutine record_final ! 最終状態を記録
        ...
        do j = 1, TYPMOL
            ...
            do k = 1, plot
                ensembleHeat(k,j) = ensembleHeat(k,j) / dble(numEnsemble)
                write(81, '(1E13.6, 2E15.7)') dble(k)*timeInterval, ensembleHeat(k,1), ensembleHeat(k,3)
            end do
        end do
