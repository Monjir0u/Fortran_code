...
!--------------- よく変更する---------------------!
    ...
    double precision, parameter :: timeRelax   = 0.010d0 ! 緩和計算時間[ns]
    ...
    double precision, parameter :: angCon = 0.03d0 ! 相互作用強さ

    character(len=20) :: dir_name = 'emd'  ! ファイル名
    ...
    integer, parameter :: numx(TYPMOL) = [20,12,20]
    integer, parameter :: numy(TYPMOL) = [10, 6,10]
    integer, parameter :: numz(TYPMOL) = [ 6,24, 6]
    integer, parameter :: nummol(TYPMOL) = [numx(1)*numy(1)*numz(1), numx(2)*numy(2)*numz(2), numx(3)*numy(3)*numz(3)] ! 各分子の数
    
    ! 境界条件
    double precision, parameter :: STDIST(TYPMOL) = [3.92d0, 5.0d0, 3.92d0] ! 格子定数(無次元)[Å]
...
program main
    ...
    subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
        ...
        do j = 1, TYPMOL
            ...
            ! ランダム力とダンパー力を追加
            do i = numx(j)*numy(j) + 1, 2*numx(j)*numy(j)         ! 加速度の無次元化 10^-20
        ...
        if(stpNow > stpScaling + stpRelax) then
            do j = 1, TYPMOL
                ...
                if(mod(stpNow-stpRelax-stpScaling, plot) == 1) then
                    zeroHeat = heatInterface(j)
                    count = 0
                end if

                do i = numx(j)*numy(j) + 1, nummol(j)
                    scalarHeat(j) = scalarHeat(j) + nowHeat(i) * zeroHeat
                end do

                count = count + 1
                scalarHeat(j) = scalarHeat(j) / (numx(j)*numy(j)*(numz(j)-1))
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        write(60,'(2E15.7)') stp*int(dt)*1.0d-6, heatfluxInterface(1)
    ...
    subroutine record_final ! 最終状態を記録
        ...
        !     end if
            do i = 1, plot
                ...
                write(81, '(1E13.6, 1E15.7)') i*timeInterval, ensembleHeat(i)
            end do
