...
subroutine record_energy_temp ! エネルギー，温度を記録
    ...
    double precision :: tempLayerPt_(numz(1),TYPMOL) ! 層ごとの温度の時間変化
    double precision :: tempLayerAr_(numDivAr) ! 領域ごとの温度の時間変化
    integer :: i, j, k
    ...
    write(30, '(I6, 4E15.7)') (stpNow)*int(dt), totEne(1), totPot(1), totKin(1)  ! energy_PtUp.dat
    write(31, '(I6, 4E15.7)') (stpNow)*int(dt), totEne(2), totPot(2), totKin(2)  ! energy_Ar.dat
    write(32, '(I6, 4E15.7)') (stpNow)*int(dt), totEne(3), totPot(3), totKin(3)  ! energy_PtLw.dat
    write(35, '(I6, 4E15.7)') (stpNow)*int(dt), allEne, allPot, allKin           ! energy_all.dat
    write(40, '(I6, 4E15.7)') (stpNow)*int(dt), temp(1), temp(2), temp(3)        ! tempe.dat

    !!!!!!!!!Pt層を増やすとき必ず変更すること!!!!!!!!!
    write(41, '(I6, 4E15.7)') (stpNow)*int(dt), tempLayerPt_(1,1), tempLayerPt_(2,1), tempLayerPt_(3,1), tempLayerPt_(4,1)
    write(42, '(I6, 15E15.7)') (stpNow)*int(dt), tempLayerAr_(1), tempLayerAr_(2), tempLayerAr_(3), tempLayerAr_(4), tempLayerAr_(5), tempLayerAr_(6), tempLayerAr_(7), tempLayerAr_(8), tempLayerAr_(9), tempLayerAr_(10), tempLayerAr_(11), tempLayerAr_(12), tempLayerAr_(13), tempLayerAr_(14), tempLayerAr_(15)
    write(43, '(I6, 4E15.7)') (stpNow)*int(dt), tempLayerPt_(1,3), tempLayerPt_(2,3), tempLayerPt_(3,3), tempLayerPt_(4,3)
...
subroutine record_pressure_heatflux ! 熱流束を記録
    ...
    write(60,'(I6, 6E15.7)') (stp)*int(dt)*1.0d-6, heatPhantom(1), heatPhantom(3), heatInterface(1), heatInterface(3)
    write(61,'(I6, 3E15.7)') (stp)*int(dt)*1.0d-6, pressure(1), pressure(2), pressure(3)
