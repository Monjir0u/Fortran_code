...
!--------------- よく変更する---------------------!
    ...
    double precision, parameter :: timeRelax   = 0.025d0 ! 緩和計算時間[ns]
    double precision, parameter :: timeMeasure = 2.0d0 ! データ計測時間[ns]
    ...
    double precision, parameter :: angCon = 0.02d0 ! 相互作用強さ

    character(len=20) :: dir_name = 'q_kakunin'  ! ファイル名
    ...
    ! 境界条件
    double precision, parameter :: STDIST(TYPMOL) = [3.92d0, 4.0d0, 3.92d0] ! 格子定数(無次元)[Å]
    ...
    ! 粒子登録法
    integer, parameter :: stpBook = 40 ! 粒子登録の更新ステップ間隔
    ...
    ! Green-Kubo用
    double precision, parameter :: timePlot = 10 ! データプロット時間 [ps]
    double precision, parameter :: timeInterval = 0.05 ! 時間間隔 [ps]

    integer, parameter :: stpPlot = int(timePlot/dt*1.0d+3) ! データプロット時間をステップ数に変換
    integer, parameter :: plot = int(timePlot/timeInterval)+1 ! プロット数
...
module variable
    ...
    ! EMD(平衡分子動力学)
    double precision :: nowHeat(nummol(1))
    double precision :: zeroHeat(nummol(1))
    double precision :: scalarHeat(TYPMOL)
    double precision :: resist(TYPMOL) ! 界面熱抵抗

    ! Green-Kubo用
    double precision :: ensembleHeat(plot+10) ! 熱流束の内積を格納する配列
    integer :: count ! ensembleHeatの配列に入れる用のカウント
...
program main
    ...
    ! 各分子の最終位置データの出力
        ...
        open(81,file=trim(filepath) // '/ensemble_heatflow.dat')
    !　分子の色
    ...
    ! ターミナルに表示
    write(6,*) ''
    write(6, '(A18, F7.4, A, I8, A)') 'Scaling Time : ', timeScaling, 'ns', stpScaling, 'step'
    write(6, '(A18, F7.4, A, I8, A)') 'Relaxation Time : ', timeRelax, 'ns', stpRelax, 'step'
    write(6, '(A18, F7.4, A, I8, A)') 'Measure Time : ', timeMeasure, 'ns', stpMeasure, 'step'
    write(6,*) ''
    ...
    do i = 1, stpMax
        ...
        if(stpNow >= stpScaling + stpRelax .and. mod(stpNow, int(tau/dt)) == 1) then
    ...
    !     implicit none
    !     integer :: i, j, k, i1, i2
    !     double precision :: vmax(), div(3), dit2, dist

    !     if(mod(stpNow,stpBook) == 1) then
    !         do j = 1, TYPMOL
    !             if(j == 2) then
    !                 cycle
    !             end if
    ...
    subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
        ...
        if(stpNow > stpScaling + stpRelax) then
            do j = 1, TYPMOL
                ...
                do i = int(nummol(j)/numz(j)) + 1, nummol(j) ! 固定層を除いたPt分子全体
                    nowHeat(i) = &
                            !    &  interForce(i,j,1)*1.000d-6 * typ(j)%mol(i)%vtmp(1)*1.000d+5 + &
                            !    &  interForce(i,j,2)*1.000d-6 * typ(j)%mol(i)%vtmp(2)*1.000d+5 + &
                               &  interForce(i,j,3)*1.000d-6 * typ(j)%mol(i)%vtmp(3)*1.000d+5

                    ! J(0)の決め方
                    ! いくつかのステップで区切る
                    if(mod(stpNow-stpRelax-stpScaling, stpPlot) == 1) then
                        zeroHeat(i) = nowHeat(i)
                        count = 0
                    end if
                    ...
                    heatInterface(j) = heatInterface(j) + nowHeat(i)
                    scalarHeat(j) = scalarHeat(j) + nowHeat(i) * zeroHeat(i)
                end do
                count = count + 1
                scalarHeat(j) = scalarHeat(j) / (nummol(j))
                ensembleHeat(count) = ensembleHeat(count) + scalarHeat(j)
            end do
        end if
    ...
    subroutine record_pos_vel ! 位置，速度を記録
        ...
        do i = 1, nummol(1)!(numz(1)-1)*int(nummol(1)/numz(1)) + 1, nummol(1) ! 固液界面層
            write(71, '(I6, 3E15.7)') i, interForce(i,1,1), interForce(i,2,1), interForce(i,3,1) ! 上Pt, Ar, 下Pt
        end do
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        do j = 1, TYPMOL
            ...
            ! do i = 1, nummol(j)
            !     if(j == 1) then
            !         pressure(1) = pressure(1) + interForce(i,1,3)*1.000d-6 / (areaPt * 1.000d-20) *1.000d-6 ! [MPa]　圧力のオーダーはあってそう
            !     else
        ...
        write(60,'(5E15.7)') stp*int(dt)*1.0d-6, heatfluxInterface(1), heatfluxInterface(3)
    ...
    subroutine record_final ! 最終状態を記録
        ...
        !     end if
            do i = 1, plot+10
                ensembleHeat(i) = ensembleHeat(i) / numEnsemble
                write(81, *) i*timeInterval, ensembleHeat(i)
            end do
            ! resist(j) = tempAr**2 * BOLTZ / (areaPt * 1.000d-20) / scalarHeat(j)
        ! end do
