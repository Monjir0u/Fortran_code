        ...
        do j = 1, TYPMOL
            ...
            do i = int(nummol(j)/numz(j)) + 1, 2*int(nummol(j)/numz(j)) ! Phantom層のみ
                do k = 1, 3
                    ...
                    ! ダンパー力
                    dmpForce(i,j,k) = -DAMP * typ(j)%mol(i)%vtmp(k) * 1.000d+5 ! 速度の有次元化
                end do
            end do
            ...
            do j = 1, TYPMOL
                ...
                do i = int(nummol(j)/numz(j)) + 1, 2*int(nummol(j)/numz(j)) ! Phantom層  
                    heatPhantom(j) = heatPhantom(j) + &
                        &  (rndForce(i,j,1) + dmpForce(i,j,1)) * typ(j)%mol(i)%vtmp(1) * 1.000d+5 + &
                        &  (rndForce(i,j,2) + dmpForce(i,j,2)) * typ(j)%mol(i)%vtmp(2) * 1.000d+5 + &
                        &  (rndForce(i,j,3) + dmpForce(i,j,3)) * typ(j)%mol(i)%vtmp(3) * 1.000d+5 ! 速さの有次元化 10^5
                end do
