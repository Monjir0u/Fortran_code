...
!--------------- よく変更する---------------------!
    ...
    character(len=20) :: dir_name = 'yuragi'  ! ファイル名
            ...
            do j = 1, TYPMOL
                ...
                do i = numx(j)*numy(j) + 1, 2*numx(j)*numy(j) ! Phantom層  
                    heatPhantom(j) = heatPhantom(j) + &
                        ! &  (rndForce(i,1,j) + dmpForce(i,1,j)) * typ(j)%mol(i)%vtmp(1) * 1.000d+5 + &
                        ! &  (rndForce(i,2,j) + dmpForce(i,2,j)) * typ(j)%mol(i)%vtmp(2) * 1.000d+5 + &
                ...
                do i = 1, nummol(j) ! Pt分子全体
                    heatInterface(j) = heatInterface(j) + &
                        ! &  interForce(i,1,j) * 1.000d-6 * typ(j)%mol(i)%vtmp(1) * 1.000d+5 + &
                        ! &  interForce(i,2,j) * 1.000d-6 * typ(j)%mol(i)%vtmp(2) * 1.000d+5 + &
                        &  interForce(i,3,j) * 1.000d-6 * typ(j)%mol(i)%vtmp(3) * 1.000d+5
                end do
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        do j = 1, TYPMOL
            ...
            module parameters
                ...
                !--------------- よく変更する---------------------!
                ! 計算時間
                double precision, parameter :: timeScaling = 0.05d0 ! スケーリング時間[ns]
                ...
                double precision, parameter :: timeInterval = 0.05d0 ! 時間間隔 [ps]
            ...
            module variable
                ...
                double precision :: heatInterface(TYPMOL) ! 固液界面での熱輸送量
                double precision :: pressure(TYPMOL) ! 圧力
            ...
            program main
                ...
                subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
                   ...
                   ! 異なる分子同士の影響  ! Ar-Ptの処理    配列は1が上Pt, 2がAr, 3がしたPt
                    ...
                    do j = 1, TYPMOL
                        ...
                        do i = numx(j)*numy(j) + 1, 2*numx(j)*numy(j) ! Phantom層のみ
                            do k = 1, 3
                                rnd = Random() 
                                ! ランダム力
                                rndForce(i,j,k) = rnd * getStddev(tempLanPt(j)) * 1.000d-9 ! 標準偏差の有次元化
                                ! ダンパー力
                                dmpForce(i,j,k) = -DAMP * typ(j)%mol(i)%vel(k) * 1.000d+5 ! 速度の有次元化
                            end do
                        end do
                    ...
                    heatInterface(:) = 0.000d0  ! 確認用に新たに追加    !!! 後で消す !!!
                    ...
                    if(stpNow > stpScaling + stpRelax) then
                        do j = 1, TYPMOL
                            ...
                            if(stpNow-stpRelax-stpScaling > plot) then
                                zeroHeat = heatInterface(j)
                                count = 0
                            end if
            
                            scalarHeat(j) = scalarHeat(j) + heatInterface(j) * zeroHeat
            
                            count = count + 1
                            if(stp < 2000) then
                                write(69,'(3I7, 2E15.7)') stpNow, stp, count, zeroHeat, heatInterface(j)
                            end if
                            ! ensembleHeat(count,j) = ensembleHeat(count,j) + scalarHeat(j)
                        end do
                    end if
                ...
                subroutine record_pos_vel ! 位置，速度を記録
                    ...
                    do i = numx(1)*numy(1) + 1, 2*numx(1)*numy(1) ! Phantom層
                        write(70, '(I6, 6E15.7)') i, rndForce(i,1,1), rndForce(i,1,2), rndForce(i,1,3), &
                                                     dmpForce(i,1,1), dmpForce(i,1,2), dmpForce(i,1,3)
                    end do
                ...
                subroutine record_final ! 最終状態を記録
                    ...
                    !     end if
                        ! do k = 1, plot
                        !     ensembleHeat(k) = ensembleHeat(k) / dble(numEnsemble)
                        !     write(81, '(1E13.6, 1E15.7)') k*timeInterval, ensembleHeat(k)
                        ! end do
                    ! end do
