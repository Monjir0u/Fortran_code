module parameters
    ...
    !--------------- よく変更する---------------------!
    ! 計算時間
    double precision, parameter :: timeScaling = 0.001d0 ! スケーリング時間[ns]
    double precision, parameter :: timeRelax   = 0.001d0 ! 緩和計算時間[ns]
    double precision, parameter :: timeMeasure = 1.0d0 ! データ計測時間[ns]
    ...
    double precision, parameter :: angCon = 0.10d0 ! 相互作用強さ

    character(len=20) :: dir_name = 'pressure'  ! ファイル名
...
module variable
    ...
    double precision :: zeroHeat(numSlide, TYPMOL)
    double precision :: scalarHeat(numSlide, TYPMOL)
...
program main
    ...
    ! 各分子の最終位置データの出力
        ...
        open(81,file=trim(filepath) // '/ensemble_heatflow_top.dat')
        open(82,file=trim(filepath) // '/ensemble_heatflow_bottom.dat')
    ...
    write(6, '(A23, F4.2)') 'Strength Coefficient : ', angCon
    write(6,'(A4,I5,A7,I5,A8,I5,A11,I5)') 'plot', plot, 'stpPlot', stpPlot, 'stpSlide', stpSlide, 'stpInterval', stpInterval
    write(6,*) numEnsemble
    write(6,*) ''
    ...
    do i = 1, stpMax + stpSlide*(numSlide-1)
        ...
        if (mod(stpNow,500) == 0) then
            write(6,'(3X, I9, 1X, A3, I9, A1, F15.7, A3)') stpNow, ' / ', stpMax + stpSlide*(numSlide-1), '(', time, ' s)'
        endif
        ...
        ! ステップ数が100の倍数+1のとき
        if(mod(stpNow, stpRecord) == 1) then
                    ...
                    if(stp > 0 .and. stp <= stpMeasure) then
                        if(mod(stp, stpInterval) == 1) then
                            if(mod(stp, stpPlot) == 1) then
                                zeroHeat(i,j) = heatInterface(j)
                                cnt(i,j) = 0
                            end if

                            scalarHeat(i,j) = scalarHeat(i,j) + heatInterface(j) * zeroHeat(i,j)

                            cnt(i,j) = cnt(i,j) + 1
                            ensembleHeat(cnt(i,j),j) = ensembleHeat(cnt(i,j),j) + scalarHeat(i,j)

                            write(69,'(2I7, 2I4, 2E15.7)') stpNow, stp, i, cnt(i,j), zeroHeat(i,j), heatInterface(j)
                        end if
                    end if
                end do
            end do
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        do j = 1, TYPMOL
            ...
            heatfluxInterface(j) = heatInterface(j) / (areaPt * 1.000d-20) ! 面積の有次元化 10^-20
    ...
    subroutine record_final ! 最終状態を記録
        ...
        do j = 1, TYPMOL
            if(j == 2) then
                do i = 1, numDivAr
                    tempLayerAr(i) = tempLayerAr(i) / dble(stpMeasure/stpRecord)
                end do
            else
                do i = 2, numz(j)
                    tempLayerPt(i,j) = tempLayerPt(i,j) / dble(stpMeasure/stpRecord)
                end do
            end if
        end do
        ...
        do j = 1, TYPMOL
            ...
            do k = 1, plot
                ...
                ensembleHeat(k,j) = ensembleHeat(k,j) / (areaPt*areaPt*1.000d-40)
                if(j == 1) then
