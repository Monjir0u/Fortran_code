...
!--------------- よく変更する---------------------!
    ! 計算時間
    double precision, parameter :: time_scaling = 0.010d0 ! スケーリング時間[ns]
    double precision, parameter :: time_relax   = 0.050d0 ! 緩和計算時間[ns]
    double precision, parameter :: time_measure = 0.05d0 ! データ計測時間[ns]
...
module variable
    ...
    double precision :: zdiv = (BND_LEN_Z0-THICK(TOP)-THICK(BTM))/NUM_DIV_Ar ! Arの温度分布の分割距離
...
program main
    ...
    ! ターミナルに表示
        ...
        write(6, '(A23, F4.2)') 'Interaction Strength : ', INTER_STG
        write(6,*) ''
    ...
    subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
        ...
        integer :: i, j, k, i1, i2
        ! 分子間相互作用
        double precision :: div(XYZ), dist
        double precision :: dit2, dit4, dit6, dit8, dit12, dit14
        double precision :: ppp, force, forVec(XYZ)
        double precision :: vel, sumvene
        ! 粒子登録法
        double precision :: vmax(5000, PtAr), rbook(5000, PtAr)
        logical :: judge_same(5000, 5000, PtAr), judge_diff(NUM_MOL(Pt), NUM_MOL(Ar), Pt)
        ...
        do j = 1, Pt
            do i = NUM_X(j)*NUM_Y(j) + 1, 2*NUM_X(j)*NUM_Y(j) ! Phantom層のみ
                do k = 1, XYZ
                    rand = Random() 
                    ! ランダム力
                    force_rand(i,j,k) = rand * getStddev(temp_Langevin(j)) * 1.000d-9 ! 標準偏差の有次元化
                    ! ダンパー力
        ...
        do j = 1, Pt
            ...
            ! その他の層
            do i = NUM_X(j)*NUM_Y(j) + 1, NUM_MOL(j)
    ...
    subroutine record_pos_vel ! 位置，速度を記録
        ...
        do i = NUM_X(TOP)*NUM_Y(TOP) + 1, 2*NUM_X(TOP)*NUM_Y(TOP) ! Phantom層
        ...
        do i = 1, NUM_MOL(TOP)!(NUM_Z(TOP)-1)*NUM_X(TOP)*NUM_Y(TOP) + 1, NUM_MOL(TOP) ! 固液界面層
    ...
    subroutine record_energy_temp ! エネルギー，温度を記録
        ...
        double precision :: temp_layer_Pt_(NUM_Z(Pt),Pt)
        ...
        !         else
        !             temp(j) = 2.0d0 * totKin(j) / (3.0d0 * dble(NUM_MOL(j)-NUM_X(j)*NUM_Y(j)) * BOLTZ)
        !         end if
        ! end do
        ...
        do j = 1, PtAr
            ...
            if(j == Ar) then
                ...
                do i = 1, NUM_MOL(j)
                    do k = 1, NUM_DIV_Ar
                        if ( (k-1)*zdiv <= (typ(j)%mol(i)%pos(3) - THICK(BTM)) .and. (typ(j)%mol(i)%pos(3) - THICK(BTM)) < k*zdiv) then
                ...
                do k = 2, NUM_Z(j) ! Ptの層の数
                    ...
                    temp_layer_Pt_(k, j) = 2.0d0 * kinPtTmp / (3.0d0 * dble(NUM_X(j)*NUM_Y(j)) * BOLTZ)
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        double precision :: flux_interface(Pt) ! 固液界面での熱輸送量
        integer :: step
        step = step_now-step_relax-step_scaling
