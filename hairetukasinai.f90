...
!--------------- よく変更する---------------------!
    ...
    double precision, parameter :: tau = 1.0d0 ! 測定間隔[fs]
    ...
    integer, parameter :: numx(TYPMOL) = [10, 8,10]
    integer, parameter :: numy(TYPMOL) = [ 5, 4, 5]
...
module variable
    ...
    ! Langevin法
    double precision, allocatable :: rndForce_tp_x(:), rndForce_tp_y(:), rndForce_tp_z(:) ! ランダム力用
    double precision, allocatable :: rndForce_bm_x(:), rndForce_bm_y(:), rndForce_bm_z(:)
    double precision, allocatable :: dmpForce_tp_x(:), dmpForce_tp_y(:), dmpForce_tp_z(:) ! ダンパー力用
    double precision, allocatable :: dmpForce_bm_x(:), dmpForce_bm_y(:), dmpForce_bm_z(:)
    ...
    double precision, allocatable :: interForce_tp_x(:), interForce_tp_y(:), interForce_tp_z(:) ! 熱流束を計算するための相互作用力
    double precision, allocatable :: interForce_bm_x(:), interForce_bm_y(:), interForce_bm_z(:)
...
program main
    ...
    allocate(rndForce_tp_x(nummol(1)), rndForce_tp_y(nummol(1)), rndForce_tp_z(nummol(1)))
    allocate(rndForce_bm_x(nummol(3)), rndForce_bm_y(nummol(3)), rndForce_bm_z(nummol(3)))
    allocate(dmpForce_tp_x(nummol(1)), dmpForce_tp_y(nummol(1)), dmpForce_tp_z(nummol(1)))
    allocate(dmpForce_bm_x(nummol(3)), dmpForce_bm_y(nummol(3)), dmpForce_bm_z(nummol(3)))
    allocate(interForce_tp_x(nummol(1)), interForce_tp_y(nummol(1)), interForce_tp_z(nummol(1)))
    allocate(interForce_bm_x(nummol(3)), interForce_bm_y(nummol(3)), interForce_bm_z(nummol(3)))
    ...
    write(6,*) ''
    write(6, '(A8, I5)') 'Ar : ', nummol(2)
    write(6, '(A8, I5, A2)') 'Pt : ', nummol(1), '*2'
    write(6, '(A8, I5)') 'total : ', tlnkoss
    write(6, '(A23, F4.2)') 'Coefficient Strength : ', angCon
    write(6,*) ''
    ...
    do i = 1, stpMax
        ...
        if (mod(stpNow,500) == 0) then
            write(6,'(3X, I7, 1X, A3, I9, A1, F15.7, A3)') stpNow, ' / ', stpMax, '(', time, ' s)'
        endif
        ...
        if(stpNow >= stpScaling + stpRelax .and. mod(stpNow, int(tau/dt)) == 0) then
    ...
    subroutine calcu ! 各分子に働く力，速度，位置の分子動力学計算
        ...
        interForce_tp_x(:) = 0.000d0
        interForce_tp_y(:) = 0.000d0
        interForce_tp_z(:) = 0.000d0
        interForce_bm_x(:) = 0.000d0
        interForce_bm_y(:) = 0.000d0
        interForce_bm_z(:) = 0.000d0
        ...
        do i1 = 1, nummol(1)       ! Pt
            do i2 = 1, nummol(2)    ! Ar
                div(:) = typ(1)%mol(i1)%pos(:) - typ(2)%mol(i2)%pos(:)
                ...
                typ(1)%mol(i1)%poten = typ(1)%mol(i1)%poten + ppp*0.500d0
                typ(2)%mol(i2)%poten = typ(2)%mol(i2)%poten + ppp*0.500d0

                force  = forCoef(4)*(-2.00d0/dit14+1.00d0/dit8)
                forVec(:) = -force*div(:)
                interForce_tp_x(i1) = interForce_tp_x(i1) - forVec(1) ! 無次元なことに注意　符号が逆な気がする
                interForce_tp_y(i1) = interForce_tp_y(i1) - forVec(2)
                interForce_tp_z(i1) = interForce_tp_z(i1) - forVec(3)

                typ(1)%mol(i1)%acc(:) = typ(1)%mol(i1)%acc(:) + forVec(:)/MASS(1)
    ...
    ! 下壁面
        do i1 = 1, nummol(1)       ! Pt
            do i2 = 1, nummol(2)    ! Ar
                div(:) = typ(3)%mol(i1)%pos(:) - typ(2)%mol(i2)%pos(:)
                ...
                dit14  = dit8*dit6
                ppp    = angCon*4.00d0*EPS(4)*(1.00d0/dit12-1.00d0/dit6) ! 異分子間ではangCon(接触角)を忘れずに
                typ(3)%mol(i1)%poten = typ(3)%mol(i1)%poten + ppp*0.500d0
                typ(2)%mol(i2)%poten = typ(2)%mol(i2)%poten + ppp*0.500d0

                force  = forCoef(4)*(-2.00d0/dit14+1.00d0/dit8)
                forVec(:) = -force*div(:)
                interForce_bm_x(i1) = interForce_bm_x(i1) - forVec(1)
                interForce_bm_y(i1) = interForce_bm_y(i1) - forVec(2)
                interForce_bm_z(i1) = interForce_bm_z(i1) - forVec(3)

                typ(3)%mol(i1)%acc(:) = typ(3)%mol(i1)%acc(:) + forVec(:)/MASS(3)
                typ(2)%mol(i2)%acc(:) = typ(2)%mol(i2)%acc(:) - forVec(:)/MASS(2)
            end do
        end do

        ! PtのPhantom層はダンパー力とランダム力を付与
        do i = int(nummol(1)/numz(1)) + 1, 2*int(nummol(1)/numz(1)) ! Phantom層のみ
            ! ランダム力
            ! rndForce(i,k,j) = rnd * getStddev(tempLanPt(j)) * 1.000d-9 ! 標準偏差の有次元化
            rnd = Random() 
            rndForce_tp_x(i) = rnd * getStddev(tempLanPt(1)) * 1.000d-9 ! 標準偏差の有次元化
            rnd = Random() 
            rndForce_tp_y(i) = rnd * getStddev(tempLanPt(1)) * 1.000d-9
            rnd = Random() 
            rndForce_tp_z(i) = rnd * getStddev(tempLanPt(1)) * 1.000d-9
            ! ダンパー力
            ! dmpForce(i,k,j) = -DAMP * typ(j)%mol(i)%vtmp(k) * 1.000d+5 ! 速度の有次元化
            dmpForce_tp_x(i) = -DAMP * typ(1)%mol(i)%vtmp(1) * 1.000d+5 ! 速度の有次元化
            dmpForce_tp_x(i) = -DAMP * typ(1)%mol(i)%vtmp(2) * 1.000d+5
            dmpForce_tp_x(i) = -DAMP * typ(1)%mol(i)%vtmp(3) * 1.000d+5
        end do

        do i = int(nummol(3)/numz(3)) + 1, 2*int(nummol(3)/numz(3)) ! Phantom層のみ
            ! ランダム力
            rnd = Random() 
            rndForce_bm_x(i) = rnd * getStddev(tempLanPt(3)) * 1.000d-9
            rnd = Random() 
            rndForce_bm_y(i) = rnd * getStddev(tempLanPt(3)) * 1.000d-9
            rnd = Random() 
            rndForce_bm_z(i) = rnd * getStddev(tempLanPt(3)) * 1.000d-9
            ! ダンパー力
            dmpForce_bm_x(i) = -DAMP * typ(3)%mol(i)%vtmp(1) * 1.000d+5
            dmpForce_bm_x(i) = -DAMP * typ(3)%mol(i)%vtmp(2) * 1.000d+5
            dmpForce_bm_x(i) = -DAMP * typ(3)%mol(i)%vtmp(3) * 1.000d+5
        end do

        ! ランダム力とダンパー力を追加
        do i = int(nummol(1)/numz(1)) + 1, 2*int(nummol(1)/numz(1))         ! 加速度の無次元化 10^-20
            ! typ(j)%mol(i)%acc(:) = typ(j)%mol(i)%acc(:) + (rndForce(i,:,j)*1.0d+9 + dmpForce(i,:,j)*1.0d+9) / MASS(j)*1.000d-3 ! -9+26-20 = -3
            typ(1)%mol(i)%acc(1) = typ(1)%mol(i)%acc(1) + (rndForce_tp_x(i)*1.0d+9 + dmpForce_tp_x(i)*1.0d+9) / MASS(1)*1.000d-3 ! -9+26-20 = -3
            typ(1)%mol(i)%acc(2) = typ(1)%mol(i)%acc(2) + (rndForce_tp_y(i)*1.0d+9 + dmpForce_tp_y(i)*1.0d+9) / MASS(1)*1.000d-3
            typ(1)%mol(i)%acc(3) = typ(1)%mol(i)%acc(3) + (rndForce_tp_z(i)*1.0d+9 + dmpForce_tp_z(i)*1.0d+9) / MASS(1)*1.000d-3
        end do

        do i = int(nummol(3)/numz(3)) + 1, 2*int(nummol(3)/numz(3))         ! 加速度の無次元化 10^-20
            typ(3)%mol(i)%acc(1) = typ(3)%mol(i)%acc(1) + (rndForce_bm_x(i)*1.0d+9 + dmpForce_bm_x(i)*1.0d+9) / MASS(3)*1.000d-3
            typ(3)%mol(i)%acc(2) = typ(3)%mol(i)%acc(2) + (rndForce_bm_y(i)*1.0d+9 + dmpForce_bm_y(i)*1.0d+9) / MASS(3)*1.000d-3
            typ(3)%mol(i)%acc(3) = typ(3)%mol(i)%acc(3) + (rndForce_bm_z(i)*1.0d+9 + dmpForce_bm_z(i)*1.0d+9) / MASS(3)*1.000d-3
        end do
        ...
        if(stpNow > stpRelax) then
            if(stpNow == stpScaling + stpRelax+1) then   ! パラメータモジュールで初期化するとうまくいかなかったのでここで初期化
                heatPhantom(:) = 0.000d0
                heatInterface(:) = 0.000d0
            end if
    
            do i = int(nummol(1)/numz(1)) + 1, 2*int(nummol(1)/numz(1)) ! Phantom層  
                heatPhantom(1) = heatPhantom(1) + &
                    ! & ((rndForce(i,1,j) + dmpForce(i,1,j)) * typ(j)%mol(i)%vtmp(1) + &
                    ! &  (rndForce(i,2,j) + dmpForce(i,2,j)) * typ(j)%mol(i)%vtmp(2) + &
                    ! &  (rndForce(i,3,j) + dmpForce(i,3,j)) * typ(j)%mol(i)%vtmp(3)) * 1.000d+5 ! 速さの有次元化 10^5
                    & ((rndForce_tp_x(i) + dmpForce_tp_x(i)) * typ(1)%mol(i)%vtmp(1) + &
                    &  (rndForce_tp_y(i) + dmpForce_tp_y(i)) * typ(1)%mol(i)%vtmp(2) + &
                    &  (rndForce_tp_z(i) + dmpForce_tp_z(i)) * typ(1)%mol(i)%vtmp(3)) * 1.000d+5 ! 速さの有次元化 10^5
            end do
            
            do i = int(nummol(3)/numz(3)) + 1, 2*int(nummol(3)/numz(3)) ! Phantom層  
                heatPhantom(3) = heatPhantom(3) + &
                    & ((rndForce_bm_x(i) + dmpForce_bm_x(i)) * typ(3)%mol(i)%vtmp(1) + &
                    &  (rndForce_bm_y(i) + dmpForce_bm_y(i)) * typ(3)%mol(i)%vtmp(2) + &
                    &  (rndForce_bm_z(i) + dmpForce_bm_z(i)) * typ(3)%mol(i)%vtmp(3)) * 1.000d+5 ! 速さの有次元化 10^5
            end do

            do i = 1, nummol(1) ! Pt分子全体
                heatInterface(1) = heatInterface(1) + &
                    & (interForce_tp_x(i) * 1.000d-6 * typ(1)%mol(i)%vtmp(1) * 1.000d+5 + &
                    &  interForce_tp_y(i) * 1.000d-6 * typ(1)%mol(i)%vtmp(2) * 1.000d+5 + &
                    &  interForce_tp_z(i) * 1.000d-6 * typ(1)%mol(i)%vtmp(3) * 1.000d+5)
            end do
            do i = 1, nummol(3) ! Pt分子全体
                heatInterface(3) = heatInterface(3) + &
                    & (interForce_bm_x(i) * 1.000d-6 * typ(3)%mol(i)%vtmp(1) * 1.000d+5 + &
                    &  interForce_bm_y(i) * 1.000d-6 * typ(3)%mol(i)%vtmp(2) * 1.000d+5 + &
                    &  interForce_bm_z(i) * 1.000d-6 * typ(3)%mol(i)%vtmp(3) * 1.000d+5)
            end do
        end if
    ...
    subroutine record_pos_vel ! 位置，速度を記録
        ...
        do i = int(nummol(1)/numz(1)) + 1, 2*int(nummol(1)/numz(1)) ! Phantom層
            write(70, '(I6, 6E15.7)') i, rndForce_tp_x(i), rndForce_tp_y(i), rndForce_tp_z(i), &
                                         dmpForce_tp_x(i), dmpForce_tp_y(i), dmpForce_tp_z(i)
        end do
    
        do i = 1, nummol(1)!(numz(1)-1)*int(nummol(1)/numz(1)) + 1, nummol(1) ! 固液界面層
            write(71, '(I6, 3E15.7)') i, interForce_tp_x(i), interForce_tp_y(i), interForce_tp_z(i) ! 上Pt, Ar, 下Pt
        end do
    ...
    subroutine record_energy_temp ! エネルギー，温度を記録
        ...
        write(30, '(5E15.7)') stp*int(dt)*1.0d-6, totEne(1), totPot(1), totKin(1)  ! energy_PtUp.dat
        write(31, '(5E15.7)') stp*int(dt)*1.0d-6, totEne(2), totPot(2), totKin(2)  ! energy_Ar.dat
        write(32, '(5E15.7)') stp*int(dt)*1.0d-6, totEne(3), totPot(3), totKin(3)  ! energy_PtLw.dat
        write(35, '(5E15.7)') stp*int(dt)*1.0d-6, allEne, allPot, allKin           ! energy_all.dat
        write(40, '(5E15.7)') stp*int(dt)*1.0d-6, temp(1), temp(2), temp(3)        ! tempe.dat
    ...
    subroutine record_pressure_heatflux ! 熱流束を記録
        ...
        pressure(1) = pressure(1) + interForce_tp_z(i)*1.000d-6 / (areaPt * 1.000d-20) *1.000d-6 ! [MPa]　圧力のオーダーはあってそう
        pressure(3) = pressure(3) - interForce_bm_z(i)*1.000d-6 / (areaPt * 1.000d-20) *1.000d-6
    ...
    subroutine record_final ! 最終状態を記録
        ...
        do j = 1, TYPMOL
            if(j == 2) then
                do i = 1, numDivAr
                    tempLayerAr(i) = tempLayerAr(i) / dble(stpMeasure/(tau/dt))
                end do
            else
                do i = 2, numz(j)
                    tempLayerPt(i,j) = tempLayerPt(i,j) / dble(stpMeasure/(tau/dt))
                end do
            end if
        end do
